...
def setup_device_and_load_networks():
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    pnet, rnet, onet = load_networks(device)
    return device, pnet, rnet, onet
...
def correct_bboxes(bboxes, width, height):
    ...
    return return_list
def detect_faces(image, min_face_size, pnet, rnet, onet, thresholds, nms_thresholds, height, width, device):
    bounding_boxes = []
    landmarks = None
...
def visualize_results(image, bounding_boxes, landmarks=None):
...
def detect_faces_realtime(pnet, rnet, onet, min_face_size, thresholds, nms_thresholds, device):
    # 웹캠 열기
    ...
    while True:
        ...
        bounding_boxes, landmarks = detect_faces(frame, min_face_size, pnet, rnet, onet, thresholds, nms_thresholds, height, width, device)
        ...
        if landmarks is not None:
            for i in range(landmarks.shape[0]):
                ...
                landmarks_one = landmarks_one.reshape(2, 5).T
                for j in range(5):
                    cv2.circle(frame, (int(landmarks_one[j, 0]), int(landmarks_one[j, 1])), 2, (0, 255, 0), 1)
...
def main():
    ...
    detect_faces_realtime(pnet, rnet, onet, min_face_size, thresholds, nms_thresholds, device)
